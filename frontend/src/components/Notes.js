...
const Notes = (props) => {
  ...
  return (
    ...
    <AddNote showAlert={props.showAlert} />
      <button ref={btnRef} type="button" className="btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#exampleModal">
        Launch demo modal
      </button>
      ...
      <div className="row my-3">
        ...
        {notes.length === 0 && "No notes to display, Add your first note from above"}
        </div>
        {notes.map((note) => {
          return <NoteItem key={note._id} note={note} openModal={openModal} showAlert={props.showAlert} />
        })}
      </div>
    </>
  );
